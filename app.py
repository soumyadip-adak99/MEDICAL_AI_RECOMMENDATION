...
DATA_PATH = "data/Doctor_data_with_diseases.csv"

svc = None
doctors_df = None

def load_data():
    global svc, doctors_df
    try:
        ...
        if os.path.exists(DATA_PATH):
            doctors_df = pd.read_csv(DATA_PATH)
        else:
...
def get_disease(patient_symptoms):
    ...
    for symptom in patient_symptoms:
        ...
        if clean_symptom in symptoms_dict:
            input_vector[symptoms_dict[clean_symptom]] = 1
            found_symptom = True
            
    if not found_symptom:
        return "Could not recognize any symptoms"
...
def suggest_doctors(disease, sort_by="both"):
    if doctors_df is None:
        return "Error: Doctor database not loaded."
    
    filtered_doctors = doctors_df[doctors_df['Diseases'].str.contains(disease, case=False, na=False, regex=False)]
...
def index():
    if request.method == 'POST':
        ...
        if predicted_disease.startswith("Error") or predicted_disease.startswith("Could not"):
             return render_template('index.html', messages=[predicted_disease])

        suggested_doctors = suggest_doctors(predicted_disease, sort_by)
        
        if isinstance(suggested_doctors, str):
            return render_template('index.html', predicted_disease=predicted_disease, messages=[suggested_doctors])
        else:
